...
export default function QRCodeDisplay({ requestId, verificationCode }) {
  const verificationUrl = `http://192.168.1.105:3000/verify/${verificationCode}`;
  ...
  return (
    <div style={{ textAlign: 'center', padding: '2rem', background: '#f3f4f6', borderRadius: '0.5rem', marginTop: '1.5rem' }}>
      ...
      <div style={{ marginTop: '1rem' }}>
        ...
        <p style={{ margin: '0', color: '#1f2937', fontWeight: '600', wordBreak: 'break-all' }}>{verificationCode}</p>
      </div>

      <button 
        onClick={downloadQR}
        style={{ marginTop: '1rem', background: '#1f2937', color: '#ffffff', border: 'none', padding: '0.5rem 1rem', borderRadius: '0.375rem', fontWeight: '600', cursor: 'pointer', fontSize: '0.875rem' }}
      >
        ⬇️ Download QR
      </button>
